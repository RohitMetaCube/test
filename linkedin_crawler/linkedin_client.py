...
class LinkdeinClient:
    ...
    def data_cleaner(self, soup):
        ...
        for k, v in self.data.items():
            try:
                ...
                elif k == "total_connections":
                    result_text = [x.getText() for x in  result_tag.contents if isinstance(x, Tag)][0]
                elif k=="experience":
    ...
    def html_fetcher(self, profile_link):
        ...
        while retry_count:
            print "Fetching Profile: {}".format(profile_link)
            ...
            if soup.find('div', { "class" : "pv-top-card-section__summary mt4 ph2 ember-view" }):
                ...
                break
            print "Need to Retry Profile: {}".format(profile_link)
            retry_count-=1
